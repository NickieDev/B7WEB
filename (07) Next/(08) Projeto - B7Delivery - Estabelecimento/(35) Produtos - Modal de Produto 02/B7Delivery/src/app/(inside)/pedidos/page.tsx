      ...
      <>
         <Box sx={{ my: 3, displayPrint: 'none' }}>
            ...
            <Grid container spacing={ 3 } columns={{ xs: 1, sm: 2, md: 4 }}>
               {/* Representa o numero de columas por tamanho */}
               { loading &&
                  <>
                     <Grid item xs={ 1 }>
         ...
         <Box sx={{ display: 'none', displayPrint: 'block' }}>
            { printOrder &&
               <>
                  ...
                  <Box>
                     Data do pedido: { dateFormat(printOrder.orderDate) }
                  </Box>

                  <Box>
                     Cliente: { printOrder.userName }
                  </Box>
                  ...
                  <Box>
                     Tipo de pagamento: { printOrder.paymentType === 'card' ? 'Cartão' : 'Dinherio' }
                  </Box>

                  <Box>
                     Subtotal: R$ { printOrder.subTotal.toFixed(2) }
                  </Box>

                  <Box>
                     Entrega: R$ { printOrder.shippingPrice.toFixed(2) }
                  </Box>

                  { printOrder.cupomDiscount &&
                     <Box>
                        Desconto: -R${ printOrder.cupomDiscount.toFixed(2) }
                     </Box>
                  }

                  <Box>
                     Total: R$ { printOrder.total.toFixed(2) }
                  </Box>
                  ...
                  <Box>
                     Rua: { printOrder.shippingAddress.address }
                  </Box>

                  <Box>
                     Número: { printOrder.shippingAddress.number }
                  </Box>

                  <Box>
                     Complemento: { printOrder.shippingAddress.complement }
                  </Box>

                  <Box>
                     CEP: { printOrder.shippingAddress.cep }
                  </Box>
                  ...
                  <Box>
                     Cidade: { printOrder.shippingAddress.city }
                  </Box>

                  <Box>
                     Estado: { printOrder.shippingAddress.state }
                  </Box>
