...
module.exports = {
   ...
   getItem: async(req, res) => {
      ...
      console.log(`UserInfo pelo post: ${userInfo}`)
   ...
   editAction: async(req, res) => {
      ...
      // TODO: Novas Imagens
      if(req.files && req.files.img) {
         const adImgs = await Ad.findById(id)  // Linha adicionada 

         // Processo igual ao de upload
         if(req.files.img.length == undefined) { // Mandou so 1 imagem é um OBJ e não array
            if(['image/jpeg', 'image/jpg', 'image/png'].includes(req.files.img.mimetype)) {
               let url = await addImage(req.files.img.data)
               adImgs.images.push({
         ...
         adImgs.images = [...adImgs.images] // Recebe as imagens em OBJ
         await adImgs.save() // Salva as novas imagens
      }
