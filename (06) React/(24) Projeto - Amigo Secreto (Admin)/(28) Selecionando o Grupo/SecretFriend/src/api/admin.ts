...
export const upadateGroup = async(eventId: number, id: number, data: UpdateGroupData): Promise<Group | false> => {
   const token = getCookie('token')
...
export const getPeople = async(eventId: number, groupId: number) => {
   ...
   const json = await req.get(`/admin/events/${ eventId }/groups/${ groupId }`, {
      headers: { 'Authorization': `Token ${ token }` }
   })
